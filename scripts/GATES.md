...
| `scripts/check-rule-data-reads.sh` | that no interpreter site reads a repriced gas number from the global that shadows it instead of from the selected `rules.gas`. Inventories every live mention of `gasColdAccountAccess`, `gasCallValue` and `gasCreate` under `Jaune/` and in `Main.lean` against the shrink-only `scripts/rule-data-allow.txt`; a new occurrence anywhere fails until it is classified. It is structural rather than an evaluation guard on purpose (goal `jaune-forks-by-construction-v1`, D-F6): once `BenvStat` carries a `Fork`, every machine that can reach the `stateGas = none` lane runs `pragueGasSchedule`, whose three repriced numbers equal these globals by `rfl`, so a site reading the global would compute the same number on every reachable machine and no `#guard` could see it. The arithmetic half of the retired `meteringGuardLegacyRules` control survives as the schedule-level rows beside `meteringGuardLegacySchedule` in `Jaune/Execution.lean`, which price the same four quantities (31,000 / 14,500 / 5,400 / 32,700) through the schedule-carrying functions the sites call and pair each with the Prague value it differs from. Negative control: replacing `gasRules.callValue` by `gasCallValue` in `Xinst.step`'s legacy CALL arm turns this gate red naming the new occurrence, and reverting only that edit restores green | 3 shadowed globals, 15 occurrences / 14 allowlist rows | sub-second |
...
of being absorbed. All four suites run (goal C activated the two static ones;
goal `jaune-amsterdam-currency-v1` activated the two that carry the
`BPO2ToAmsterdamAtTime15k` label, rewriting goal C's refusal into the admission
with the old statement named in the harness):
...
suite derivation, `--dir` rules, the absence of every retired refusal and the two
manifest-drift controls; the transition suite's admission and dispatch, and
the `--dir` count rule on the transition tree, are pinned there too.
...
`check-rule-data-reads.sh` takes no lock and needs nothing built: it reads
tracked Lean source and one committed allowlist, and writes nothing.
...
`--suite amsterdam-smoke` does not; `--suite amsterdam-full` takes it as the
union of a heavy suite; `--suite amsterdam-transitions` does not when
sequential, exactly as the mainnet lane's `transitions`.
