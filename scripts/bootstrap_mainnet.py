...
scripts/env_doctor.py (--mainnet-deep).
"""
...
def _safe_relative_path(value: object, field: str) -> Path:
    if not isinstance(value, str) or not value:
        raise CurrentMainnetBootstrapError(f"manifest field {field} must be a nonempty string")
...
def inspect_installation(
    ...
    if mainnet_root.is_symlink():
        return InstallationState(
            True,
            False,
            (f"install root is a symbolic link, which is not accepted: {mainnet_root}",),
        )
    if _path_exists(mainnet_root) and not mainnet_root.is_dir():
        return InstallationState(
            True,
            False,
            (f"install root exists but is not a directory: {mainnet_root}",),
        )
...
def _download_archive(url: str, part_path: Path, expected_sha: str) -> None:
    ...
    try:
        ...
        raise CurrentMainnetBootstrapError(f"could not download archive from {url}: {error}") from error
    ...
    with tarfile.open(archive_path, mode="r|gz") as archive:
        for member in archive:
            try:
                ...
                raise CurrentMainnetBootstrapError(f"refusing unsafe archive: {error}") from error
...
def install_fresh(manifest: dict, mainnet_root: Path, archive_cache: Path) -> None:
    ...
    try:
        mainnet_root.mkdir(parents=True, exist_ok=True)
    except OSError as error:
        raise CurrentMainnetBootstrapError(
    ...
    try:
        staging = Path(
            tempfile.mkdtemp(prefix=f".{fixtures_root.name}.bootstrap-", dir=mainnet_root)
        )
    except OSError as error:
        raise CurrentMainnetBootstrapError(
            f"cannot create a temporary staging directory in {mainnet_root}: {error}"
        ) from error
...
def print_dry_run(
    ...
    print(
        "Would verify expected top-level dirs and release provenance"
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "(default: <mainnet-root>/<archive_filename>)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main(argv: list[str]) -> int:
    ...
    if state.correct:
        ...
        print(f"Verified install root: {mainnet_root}")
        print("No network or filesystem changes were made.")
        return 0
    ...
    if fixtures_root.exists():
        ...
        print(
            ...
            "--mainnet-root. This command will not repair, delete, or overwrite it.",
            file=sys.stderr,
        )
        return 1
