...
/-- **Jaune's SHA-256 is SHA-256 as published.**

`Bytes.sha256` — the optimized kernel in `Jaune/Hash.lean`, padding included —
computes exactly the function FIPS 180-4 defines, on every input.

Read the right-hand side against the standard rather than against the kernel:
that is what it is for, and checking it is the one thing this theorem cannot do
for a reader. `TRUSTED.md` states what the theorem does and does not claim. -/
theorem Bytes.sha256_eq_fips (m : Bytes) : Bytes.sha256 m = SHA256.FIPS.hash m := by
  show SHA256.run m = SHA256.FIPS.hash m
  rw [SHA256.run, SHA256.FIPS.hash, SHA256.FIPS.hashWords,
    SHA256.toChunks_eq _ (SHA256.length_toBytes _) m.length m rfl,
    SHA256.foldl_consumeChunk_eq, SHA256.initChunk_eq, SHA256.FIPS.pad,
    Nat.mul_comm 8 m.length]
